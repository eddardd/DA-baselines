...
def load_mnist(base_path):
    print('load_mnist')
    mnist_data = loadmat(os.path.join(base_path,
                                      "mnist_data.mat"))
    ...
    # random sample 25000 from train dataset and random sample 9000
    # from test dataset
...
def load_mnist_m(base_path):
    print('load mnist_m')
    mnistm_data = loadmat(os.path.join(base_path,
    ...
    # random sample 25000 from train dataset and random sample
    # 9000 from test dataset
...
def load_svhn(base_path):
    print('load svhn')
    svhn_train_data = loadmat(os.path.join(base_path,
                                           "svhn_train_32x32.mat"))
    svhn_test_data = loadmat(os.path.join(base_path,
                                          "svhn_test_32x32.mat"))
...
def load_syn(base_path):
    ...
    syn_train_data = loadmat(os.path.join(base_path,
                                          "synth_train_32x32.mat"))
    print("load syn test")
    syn_test_data = loadmat(os.path.join(base_path,
...
def load_usps(base_path):
    print('load usps')
    usps_dataset = loadmat(os.path.join(base_path,
                                        "usps_28x28.mat"))
...
def digit5_dataset_read(base_path, domain, batch_size):
    if domain == "mnist":
        (train_image, train_label,
         test_image, test_label) = load_mnist(base_path)
    elif domain == "mnistm":
        (train_image, train_label,
         test_image, test_label) = load_mnist_m(base_path)
    elif domain == "svhn":
        (train_image, train_label,
         test_image, test_label) = load_svhn(base_path)
    elif domain == "syn":
        (train_image, train_label,
         test_image, test_label) = load_syn(base_path)
    elif domain == "usps":
        (train_image, train_label,
         test_image, test_label) = load_usps(base_path)
    else:
    ...
    train_dataset = DigitFiveDataset(data=train_image,
                                     labels=train_label,
                                     transform=transform)
    train_loader = torch.utils.data.DataLoader(dataset=train_dataset,
                                               batch_size=batch_size,
                                               shuffle=True)
    test_dataset = DigitFiveDataset(data=test_image,
                                    labels=test_label,
                                    transform=transform)
...
class DigitFiveDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if img.shape[0] != 1:
            # transpose to Image type,so that the transform function
            # can be used
            img = Image.fromarray(
